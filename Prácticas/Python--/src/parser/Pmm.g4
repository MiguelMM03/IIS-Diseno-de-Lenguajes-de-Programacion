...
main_function returns [FunctionDef ast] locals[List<VariableDef> params=new ArrayList<>(),List<Statement> body=new ArrayList<>()]:
            'def'MAIN='main''('')'':''{'  (v=var_definition{$params.addAll($v.ast);})*(s=statement{$body.add($s.ast);})*'}'
...
;
var_definition_aux returns[List<VariableDef> ast = new ArrayList<VariableDef>()] locals [Map<String,Expression> expressions=new HashMap<>();]:
            e1=expression{$expressions.put($e1.text, $e1.ast);}(','e2=expression{$expressions.put($e2.text,$e2.ast);})*':'t=type
            {
                for(Map.Entry<String, Expression> entry:$expressions.entrySet()){
                    $ast.add(new VariableDef(entry.getValue().getLine(),entry.getValue().getColumn(),entry.getKey(),$t.ast));
                }
            }
;
function_definition returns[FunctionDef ast] locals[List<VariableDef> params=new ArrayList<>(), List<Statement> body=new ArrayList<>(), Type returnType=VoidType.getInstance()]:
            ...
            (v3=var_definition{$params.addAll($v3.ast);})*(s=statement{$body.add($s.ast);})*('return't2=type';')?'}'
            {$ast=new FunctionDef($ID.getLine(),$ID.getCharPositionInLine()+1,$ID.text,$returnType,$params,$body);}
;
statement returns [Statement ast] locals[List<Statement> elseBody=new ArrayList<Statement>()]:
            'print'e1=expression(','e2=expression)*';'
        |   'input'e1=expression(','e2=expression)*';'
        |   e1=expression OP='=' e2=expression';'{$ast=new Asignment($OP.getLine(),$OP.getCharPositionInLine()+1,$e1.ast,$e2.ast);}
        |   IF='if'e=expression':'(ifBody=body_aux)('else'':'(elseBody_aux=body_aux){$elseBody=$elseBody_aux.ast;})?{$ast=new Conditional($IF.getLine(),$IF.getCharPositionInLine()+1,$e.ast,$ifBody.ast,$elseBody);}
        |   W='while'e=expression':'body=body_aux{$ast=new While($W.getLine(),$W.getCharPositionInLine()+1,$e.ast,$body.ast);}
        |   R='return'e=expression';'{$ast=new Return($R.getLine(),$R.getCharPositionInLine()+1,$e.ast);}
        |   e1=expression'('p=params_aux')'';'{$ast=new FunctionCall($e1.ast.getLine(),$e1.ast.getColumn(),$e1.text,$p.ast);}
        |   st=statement'('')'';'{$ast=new FunctionCall($e1.ast.getLine(),$e1.ast.getColumn(),$e1.text,new ArrayList<Expression>());}
;
...
body_aux returns [List<Statement> ast = new ArrayList<>()]:
    '{'(s1=statement{$ast.add($s1.ast);})*'}'| s2=statement{$ast.add($s2.ast);}
;
expression returns [Expression ast] locals [List<Expression> params=new ArrayList<>()]:
        ...
        |   e1=expression'('(e2=expression{$params.add($e2.ast);}(','e3=expression {$params.add($e3.ast);})*)?')'{$ast = new FunctionCall($e1.ast.getLine(), $e1.ast.getColumn(), $e1.text, $params);}
...
type returns [Type ast] locals[List<RecordField> records=new ArrayList<>()]:
    ...
    |   '['INT_CONSTANT']'t=type{$ast=new ArrayType($t.ast.getLine(),$t.ast.getColumn(),LexerHelper.lexemeToInt($INT_CONSTANT.text),$t.ast);}
    |   STRUCT='struct''{'(v=var_definition
        {
            for(VariableDef vdef:$v.ast){
                $records.add(new RecordField(vdef.getName(),vdef.getType()));
            }
        }
